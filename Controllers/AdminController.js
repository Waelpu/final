...
const postAdmin = async (request, response) => {
  ...
  try {
    ...
    if (searchedAdmin) {
      return response.status(400).json({ errors:[{msg:"cet identifiant existe déjà"}] })
    }
...
const getAllAdmins = async (request, response) => {
  try {
    ...
    response.status(500).json({ error: "opération de récupération de l'administrateur échouée" })
  }
}
//delete request
...
try {
   const updatedAdmin =await Admin.findByIdAndUpdate(id,newAdmin,{new:true})
   response.status(200).json({admin:updatedAdmin, message:'Mise à jour de l\'administrateur effectuée avec succès'})
} catch (error) {
 console.log("la mise à jour de l`'administrateur n'a pas pu être effectuée",error)
...
module.exports = { postAdmin, getAllAdmins ,deleteAdmin,updateAdmin,getOneAdmin}
